...
var nome = "Fiap";
console.log(nome, " chamando variável criada com var"); //nome = "Avanade";

escreve("Exemplo de função nominal"); //função nominal - udf

function escreve(titulo) {} //alert(titulo);
...
mudaTitulo("Vai Corinthians"); // expressão de função - Function Expression
//função anônima

var mudaTitulo2 = function mudaTitulo2(target, novoTitulo) {
  document.querySelector(target).innerHTML = novoTitulo;
};

mudaTitulo2("h1", "Trocando o título"); //  Arrow Function - É uma função criada no ES6 ou ES2015
// não existe o this dentro de uma arrow function.
// não existe um método construtor.
// sintaxe mais simples e prática.

var relogio = function relogio() {
  var date = new Date();
  mudaTitulo2("h2", date.toLocaleTimeString());
};

setInterval(relogio, 1000); // setInterval(() => {},tempo);

var frases = ["A vida é bela", "A vida é curta", "A vida é uma dádiva"];
